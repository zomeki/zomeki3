...
class Cms::Controller::Script::Publication < ApplicationController
  ...
  def publish_more(item, params = {})
    ...
    first.upto(limit) do |p|
      ...
      unless rs
        ...
        if period == 'simple'
          ...
          stop_publish = true if params[:end_at].blank?
          stop_publish = true if params[:start_at].blank?
          stop_publish = true if (params[:end_at] && params[:end_at] > date)
        end
        if stop_publish
          stopp = p
          break
    ...
    first.upto(100) do |dp|
      ...
      month_num = page_number('monthly', month_date, dp)
      week_num  = page_number('weekly', week_date, dp)
