...
class Cms::Public::OAuthController < ApplicationController
  ...
  def callback
    oa_params = request.env['omniauth.params'].dup
    oa_params.each{|k, v| oa_params[k] = CGI.unescape(v) if v.index('%') }
    oa_auth = request.env['omniauth.auth']

    case oa_auth[:provider]
    when 'facebook'
      facebook_callback(oa_auth, oa_params)
    when 'twitter'
      twitter_callback(oa_auth, oa_params)
    else
  ...
  def facebook_callback(oa_auth, oa_params)
    auth = {
      provider:              oa_auth['provider'],
      uid:                   oa_auth['uid'],
...
#DEPRECATED: oauth for public bbs
#    if oa_params.empty?
...
#    end
    ...
    if oa_params['class'] == 'SnsShare::Account' && oa_params['id'].present? && oa_params['return_to'].present?
      ...
      redirect_to(oa_params['return_to'])
    else
      redirect_to(request.base_url)
    end
  end

  def twitter_callback(oa_auth, oa_params)
    auth = {
      provider:          oa_auth['provider'],
      uid:               oa_auth['uid'],
      info_nickname:     oa_auth['info']['nickname'],
